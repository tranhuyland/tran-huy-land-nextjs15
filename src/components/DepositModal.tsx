  ...
  return (
    <div class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div class="bg-white w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl p-6 relative">
        <button onClick={onClose} class="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
          <X class="w-4 h-4" />
        </button>
        <h3 class="font-extrabold text-slate-900 text-base mb-1 flex items-center gap-2">
          <PenTool class="text-amber-500 w-4 h-4" /> Ký Gửi Nhanh Trong 10s
        </h3>
        ...
        <form onSubmit={xuLyGuiFormKyGui} class="space-y-3 text-sm">
          <div>
            <label class="block font-bold text-slate-600 mb-1">Tên & SĐT Liên Hệ *</label>
            <input type="text" value={ten} onChange={e => setTen(e.target.value)} required placeholder="Ví dụ: Anh Nam - 0905..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-amber-500" />
          </div>
          <div>
            <label class="block font-bold text-slate-600 mb-1">Địa Chỉ Nhà Đất Ký Gửi *</label>
            <input type="text" value={diachi} onChange={e => setDiachi(e.target.value)} required placeholder="Số nhà, tên đường, tên quận..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-amber-500" />
          </div>
          <div>
            <label class="block font-bold text-slate-600 mb-1">Giá Bán Mong Muốn</label>
            <input type="text" value={gia} onChange={e => setGia(e.target.value)} placeholder="Ví dụ: 3.5 Tỷ (Để trống nếu muốn thương lượng)" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-amber-500" />
          </div>
