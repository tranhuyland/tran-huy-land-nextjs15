    ...
    <>
        ...
        <div class="bg-white p-5 sm:p-6 rounded-3xl border border-slate-100 shadow-xl space-y-4">
          <div class="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
            <div>
              ...
              <select value={khuVuc} onChange={e => { setKhuVuc(e.target.value); setCurrentPage(1); }} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
            ...
            <div>
              ...
              <select value={loaiHinh} onChange={e => { setLoaiHinh(e.target.value); setCurrentPage(1); }} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
            ...
            <div>
              ...
              <select value={gia} onChange={e => { setGia(e.target.value); setCurrentPage(1); }} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
            ...
            <div>
              ...
              <select value={huong} onChange={e => { setHuong(e.target.value); setCurrentPage(1); }} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
          ...
          <div class="flex flex-wrap gap-2 pt-3 border-t border-slate-100 items-center">
            ...
            <button onClick={() => { setActiveTag("all"); setCurrentPage(1); }} class={`text-xs font-bold px-4 py-2 rounded-xl transition-all ${activeTag === "all" ? 'bg-slate-900 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600'}`}>Tất Cả</button>
            <button onClick={() => { setActiveTag("mattien"); setCurrentPage(1); }} class={`text-xs font-bold px-4 py-2 rounded-xl transition-all ${activeTag === "mattien" ? 'bg-slate-900 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:border-amber-500'}`}>Mặt Tiền Kinh Doanh</button>
            <button onClick={() => { setActiveTag("chinhchu"); setCurrentPage(1); }} class={`text-xs font-bold px-4 py-2 rounded-xl transition-all ${activeTag === "chinhchu" ? 'bg-slate-900 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:border-amber-500'}`}>Hàng Chính Chủ</button>
          </div>
        </div>
      </section>
      ...
      <main id="listing-section" class="max-w-7xl mx-auto w-full px-4 mt-16 mb-20 flex-1">
        ...
        {displayedItems.length === 0 ? (
          <div class="text-center py-16 text-slate-400 text-sm font-medium">Không tìm thấy sản phẩm nhà đất nào phù hợp với bộ lọc hiện tại.</div>
        ) : (
        ...
        {totalPages > 1 && (
          <div class="col-span-full flex justify-center items-center gap-2 mt-8">
            {Array.from({ length: totalPages }, (_, idx) => idx + 1).map((p) => (
              <button key={p} onClick={() => { setCurrentPage(p); window.scrollTo({ top: (document.getElementById("listing-section")?.offsetTop || 0) - 90, behavior: "smooth" }); }} class={`w-9 h-9 rounded-xl text-sm transition-all font-bold ${p === currentPage ? 'bg-amber-500 text-slate-900 shadow-sm font-extrabold scale-105' : 'bg-white border border-slate-200 text-slate-600'}`}>
                {p}
              </button>
            ))}
          </div>
      ...
      {soDoUrl && (
        <div class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <button onClick={() => setSoDoUrl(null)} class="absolute top-4 right-4 z-50 w-10 h-10 bg-white/20 hover:bg-white/40 text-white rounded-full flex items-center justify-center transition-all">
            <X class="w-6 h-6" />
          </button>
          <div class="max-w-3xl w-full max-h-[85vh] flex items-center justify-center overflow-hidden rounded-xl">
            <img src={soDoUrl} alt="Sổ đỏ" class="max-w-full max-h-[85vh] object-contain shadow-2xl rounded-xl" />
          </div>
        </div>
      )}
    </>
