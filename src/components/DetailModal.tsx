      ...
      <div class="bg-white w-full sm:max-w-xl rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl relative max-h-[92vh] sm:max-h-[88vh] flex flex-col">
        ...
        <div class="overflow-y-auto flex-1 no-scrollbar">
          <div class="w-full relative group/slide aspect-[16/10] bg-slate-100">
            <div ref={setScrollContainer} class="w-full h-full flex overflow-x-auto snap-x snap-mandatory no-scrollbar">
              {item.videoUrl && (
                <div class="w-full h-full flex-shrink-0 snap-start snap-always relative">
                  <iframe class="w-full h-full" src={item.videoUrl} allowfullscreen></iframe>
                </div>
              )}
              {danhSachAnh.map((url, i) => (
                <div key={i} class="w-full h-full flex-shrink-0 snap-start snap-always">
            ...
            {tongSoMuc > 1 && (
              <>
                <button onClick={() => slideAction("left")} class="absolute left-3 top-1/2 -translate-y-1/2 z-50 w-9 h-9 bg-black/40 hover:bg-black/60 backdrop-blur-sm text-white rounded-full flex items-center justify-center transition-all opacity-90 active:scale-90">
                  <ChevronLeft class="w-5 h-5" />
                </button>
                <button onClick={() => slideAction("right")} class="absolute right-3 top-1/2 -translate-y-1/2 z-50 w-9 h-9 bg-black/40 hover:bg-black/60 backdrop-blur-sm text-white rounded-full flex items-center justify-center transition-all opacity-90 active:scale-90">
                  <ChevronRight class="w-5 h-5" />
                </button>
                <div class="bg-slate-900/60 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-md absolute top-4 left-4 z-10 flex items-center gap-1 shadow-sm uppercase tracking-wider">
                  <Layers class="w-3 h-3 text-amber-400" /> Giỏ hàng: {item.videoUrl ? '1 Video & ' : ''}{danhSachAnh.length} Ảnh
                </div>
              </>
            )}
          </div>
          ...
          <div class="p-6">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs text-slate-400 font-bold uppercase tracking-wider bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100 flex items-center">
                <ShieldCheck class="w-4 h-4 text-emerald-500 mr-1" />{item.phapLy || 'Sổ hồng sẵn sàng'}
              </span>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-5">
              {item.linkMap && <a href={item.linkMap} target="_blank" rel="noreferrer" class="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold border border-emerald-200 rounded-xl py-2.5 px-3 text-center text-xs sm:text-sm flex items-center justify-center gap-1.5 transition-colors shadow-sm"><Map class="w-4 h-4" /> Bản Đồ Vị Trí</a>}
              {item.anhSoDo && <button onClick={() => onOpenSoDo(item.anhSoDo!)} class="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold border border-indigo-200 rounded-xl py-2.5 px-3 text-center text-xs sm:text-sm flex items-center justify-center gap-1.5 transition-colors shadow-sm"><FileText class="w-4 h-4" /> Sổ Đỏ Bản Vẽ</button>}
            </div>
            ...
            <div class="flex gap-3 mt-4 border-t border-slate-100 pt-4">
              ...
              <a href="https://zalo.me/0931555551" target="_blank" rel="noreferrer" class="flex-1 bg-[#0068ff] hover:opacity-90 text-white font-extrabold rounded-xl py-3 px-4 flex items-center justify-center text-sm transition-all active:scale-95 shadow-md">Liên Hệ Zalo</a>
            </div>
          </div>
        </div>
      </div>
